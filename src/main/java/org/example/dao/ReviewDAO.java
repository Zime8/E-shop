...
public final class ReviewDAO {
    ...
    public List<Review> listByProductShop(long productId, int idShop) {
        try {
            if (Session.isDemo()) {
    ...
    public void upsertReview(
        ...
        try {
            ...
            try (Connection conn = DatabaseConnection.getConnection();
                ...
                cs.setInt(3, userId);
                cs.setInt(4, rating);
                if (cleanTitle == null) cs.setNull(5, Types.VARCHAR);
                else cs.setString(5, cleanTitle);
                if (cleanComment == null) cs.setNull(6, Types.VARCHAR);
                else cs.setString(6, cleanComment);
                cs.executeUpdate();
            }
        } catch (SQLException e) {
