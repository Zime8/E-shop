...
import org.example.models.dto.DisplayData;
import org.example.models.dto.ItemView;
...
public class OrderSummaryController {
    ...
    private void refreshView() {
        ...
        currentViewData = appController.buildDisplayData();
    ...
    @FXML
    private void onPay() {
        if (currentViewData == null || currentViewData.rows() == null || currentViewData.rows().isEmpty()) {
            new Alert(Alert.AlertType.WARNING, "Nessun dato carrello disponibile.").showAndWait();
            return;
        }

        if (navigator == null || appController == null) {
            new Alert(Alert.AlertType.ERROR, "Impossibile procedere al pagamento.").showAndWait();
            return;
        }

        navigator.openModal("/fxml/PaymentSelection.fxml", (PaymentSelectionController controller) -> {
