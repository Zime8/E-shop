...
public class CvvTableCell extends TableCell<CardViewModel, String> {
    ...
    public CvvTableCell(TableView<CardViewModel> table, Map<Integer, String> transientCvvs) {
        this.table = table;
        ...
        tf.textProperty().addListener((obs, oldV, newV) -> {
            ...
            if (!digits.equals(newV)) {
                Platform.runLater(() -> tf.setText(digits));
                return; }
        ...
        tf.focusedProperty().addListener((obs, wasFocused, isFocused) -> {
            if (!isFocused && getTableRow() != null && getTableRow().getItem() != null) {
                CardViewModel row = getTableRow().getItem();
                String cvv = tf.getText().trim();
                if (cvv.isEmpty()) transientCvvs.remove(row.getId());
                else transientCvvs.put(row.getId(), cvv);
            }
        });
    }
    ...
    private CardViewModel currentRow() {
